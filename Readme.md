...
```sh
gst-launch-1.0 -e -v uridecodebin uri=file://$(pwd)/subject/camera.mp4 ! videoconvert ! "video/x-raw, format=(string)RGB" ! cudafilter opening_size=3 th_low=15 th_high=30 bg_sampling_rate=700 bg_number_frame=15 ! videoconvert ! fpsdisplaysink
```

image de fond  = ./subject/camera.mp4
```sh
gst-launch-1.0 -e -v uridecodebin uri=file://$(pwd)/subject/camera.mp4 ! videoconvert ! "video/x-raw, format=(string)RGB" ! cudafilter bg=\"./subject/bg.ppm\" opening_size=3 th_low=15 th_high=30 bg_sampling_rate=700 bg_number_frame=15 ! videoconvert ! fpsdisplaysink
